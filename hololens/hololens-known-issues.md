---
title: Kända problem för HoloLens (första gen)
...
>[!NOTE]
> - Om du upptäcker ett problem som inte blockerar kan du rapportera det på din HoloLens via [Feedbackhubben](hololens-feedback.md).
...
1. Överst i filen lägger du till följande i den befintliga listan över `using` -instruktioner:
...
1. Kör den körbara filen och ange enhetens IP-adress som ett kommandoradsargument. (Om du är ansluten via USB kan du använda 127.0.0.1, annars använder du enhetens Wi-Fi IP-adress.)  Till exempel "HoloLensDeploymentFix 127.0.0.1".

1. När verktyget har avslutats utan meddelanden (detta bör bara ta några sekunder) kan du nu distribuera och felsöka från och med Visual Studio 2017 eller senare.  Det är inte nödvändigt att fortsätta använda verktyget.
...
Det kan uppstå problem när du försöker starta Microsoft Store appar på HoloLens. Vi har fastställt att problemet uppstår när uppdateringar av bakgrundsappen distribuerar en nyare version av ramverkspaket i specifika sekvenser medan en eller flera av deras beroende appar fortfarande körs. I det här fallet levererade en automatisk appuppdatering en ny version av .NET Native Framework (version 10.0.25531 till 10.0.27413) vilket gjorde att de appar som kördes inte uppdaterades korrekt för alla appar som kördes som förbrukar den tidigare versionen av ramverket.  Flödet för framework-uppdateringen är följande:

1. Det nya ramverkspaketet laddas ned från store och installeras.

1. Alla appar använder det äldre ramverket "uppdateras" för att använda den nyare versionen.

Om steg 2 avbryts innan det slutförs kommer alla appar som det nyare ramverket inte har registrerats för inte att starta från Start-menyn.  Vi tror att alla appar HoloLens kan påverkas av det här problemet.
...
Vi har rotorsaken till att det här problemet inte orsakade själva uppdateringen, utan en bugg i operativsystemet som resulterade i att .NET Native framework-uppdateringen hanterades felaktigt. Vi är glada över att kunna meddela att vi har identifierat en korrigering och har släppt en uppdatering (OS-version 17763.380) som innehåller korrigeringen.  

Så här ser du om enheten kan ta uppdateringen:

1. Gå till Inställningar och öppna **Update & Security**.

1. Välj **Sök efter uppdateringar.**

1. Om uppdatering till 17763.380 är tillgänglig uppdaterar du till det här bygget för att få korrigeringen för appens hang-bugg.

1. När du uppdaterar till den här versionen av operativsystemet bör apparna fungera som förväntat.

Som vi gör med varje version HoloLens operativsystem har vi dessutom publicerat FFU-avbildningen till [Microsoft Download Center.](https://aka.ms/hololensdownload/10.0.17763.380)
...
1. Kontrollera att enheten är upplåst av utvecklaren.  Om du inte har gjort det tidigare kan du läsa [använda Windows Enhetsportalen](/windows/mixed-reality/using-the-windows-device-portal) anvisningar.
...
- Om du startar om HoloLens xbox trådlösa styrenheten är ansluten, kommer styrenheten inte automatiskt återansluta till HoloLens. Guideknappen blinkar långsamt tills styrenheten är avstängd efter 3 minuter. Om du vill återansluta kontrollanten direkt stänger du av styrenheten genom att hålla ned guideknappen tills lampan stängs av. När du sätt på styrenheten igen återansluts den till HoloLens.

- Om enheten HoloLens i vänteläge när den trådlösa Xbox-styrenheten är ansluten aktiveras alla indata på styrenheten HoloLens. Du kan förhindra detta genom att stänga av styrenheten när du är klar med den.
