...
1. **Helskärmsläge för enskild** app – Ingen Start-meny visas och en enskild app startas automatiskt när användaren loggar in. <br> *I exemplet* används : En enhet som bara kör Appen Dynamics 365-guider.
...
- Helskärmsläge blockerar inte en app (konfigurerad som en del av helskärmsläget) från att starta andra appar. Om du helt vill blockera start av vissa appar/processer på HoloLens kan du gå till Använda Windows Defender Application Control på [HoloLens 2 enheter i Microsoft Intune – Azure](/mem/intune/configuration/custom-profile-hololens)
...
- En tilldelad åtkomstkonfiguration refererar till en profil och målanvändare för profilen, till exempel en specifik användare, en AAD-grupp eller besökare osv. Du kan definiera flera konfigurationer i samma XML-struktur beroende på komplexiteten i dina användningsscenarier (se scenarier som stöds nedan).
...
- Både AAD- och Icke-ADD-konfigurationer stöder besökarkonton som automatiskt har aktiverats för helskärmsläge.
...
- Från och [Windows Holographic version 21H1](hololens-release-notes.md#windows-holographic-version-21h1)söker helskärmsläge efter global tilldelad åtkomst innan en tom startmeny visas. Helskärmsupplevelsen kommer att gå tillbaka till en global kioskkonfiguration (om sådan finns) om det uppstår fel i helskärmsläget för AAD-grupper.
...
- Om helskärmskonfigurationen baseras på AAD-grupper ska du se till att Internetanslutningen finns när AAD-användaren loggar in. Om du vill [konfigurerar du principen MixedReality/AADGroupMembershipCacheValidityInDays](/windows/client-management/mdm/policy-csp-mixedreality#mixedreality-aadgroupmembershipcachevalidityindays) så att den även kan fungera utan Internet.
...
### <a name="issue--multiple-app-assigned-access-to-aad-group-does-not-work"></a>Problem – Flera appar har tilldelats åtkomst till AAD-gruppen fungerar inte
...
- I konfigurations-XML för tilldelad åtkomst bekräftar du att GUID för AAD-gruppen som den inloggade användaren är medlem i används och inte GUID för AAD-användaren.
