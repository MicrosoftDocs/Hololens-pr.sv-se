...
2. **Alternativ 2:** [Från Windows Assessment and Deployment Kit (ADK) för Windows 10](https://developer.microsoft.com/windows/hardware/windows-assessment-deployment-kit). Om du Windows Configuration Designer från Windows ADK väljer du **Configuration Designer** i dialogrutan Välj de funktioner som du **vill** installera. Det här alternativet innehåller HoloLens två funktioner.

> [!NOTE]
> Om du vet att du kommer att använda en offlinedator som behöver åtkomst till Windows Configuration Designer följer du instruktionerna [offline app install(hololens-recovery.md#downloading-arc-without-using-the-app-store) för Advanced Recovery Companion. Gör Windows ditt val i Configuration Designer. 
...
6. Expandera **Körningsinställningar och** anpassa paketet med någon av de inställningar som [beskrivs senare i den här artikeln.](#what-you-can-configure)
    ...
    > [!IMPORTANT]
    > (Endast Windows 10 version 1607) Om du skapar ett lokalt konto i etableringspaketet måste du ändra lösenordet med hjälp **av Inställningar-appen** var 42:e dag. Om lösenordet inte ändras under den perioden kan kontot vara utelåst och inte kunna logga in. Om användarkontot är utelåst måste du [utföra en fullständig enhetsåterställning.](https://developer.microsoft.com/windows/mixed-reality/reset_or_recover_your_hololens#perform_a_full_device_recovery)
...
HoloLens 2 enheter [på Windows Holographic version 20H2](hololens-release-notes.md#windows-holographic-version-20h2) eller senare har nyare funktioner för att förenkla den här processen, vilket gör den automatisk. Läs följande avsnitt:
...
7. Du ser om paketet har tillämpats eller inte. Om det inte gick kan du åtgärda paketet och försöka igen. Om det lyckades fortsätter du med OOBE.
...
> [!NOTE]
> De här stegen gäller för HoloLens 2 enheter och HoloLens (första generationens) enheter på Windows Holographic, version 1809 och senare.
...
2. Anslut HoloLens till en dator med hjälp av en USB-kabel. HoloLens visas som en enhet i Utforskaren på datorn.
3. Dra och släpp etableringspaketet till mappen Dokument på HoloLens.

Följ HoloLens på din dator:
1. Gå till **Inställningar** > **Konton** > **Åtkomst till arbete eller skola**. 
2. I **Related Inställningar** väljer du Lägg till eller ta bort ett **etableringspaket**.
...
Konfigurationstjänstleverantörer (CPS) används för att etablera paket. Om du inte är bekant med CPS kan du gå till [Introduktion till konfigurationstjänstleverantörer (CPS) för IT-proffs.](/windows/configuration/provisioning-packages/how-it-pros-can-use-configuration-service-providers)

När Windows i Configuration Designer skapar ett **konfigurationspaket** för Windows Holographic baseras inställningarna i Tillgängliga anpassningar på CPS som stöds i [Windows Holographic](/windows/client-management/mdm/configuration-service-provider-reference#csps-supported-in-hololens-devices). I följande tabell beskrivs de inställningar som du kanske vill konfigurera för HoloLens.
