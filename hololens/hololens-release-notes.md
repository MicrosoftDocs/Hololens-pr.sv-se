...
- Enhetsportalen har förbättrade metoder för att meddela kunden när Utforskaren problem med att öppna låsta filer.
...
Vi har lagt till en ny funktion i HoloLens 2 Inställningar-appen, som gör att kunder automatiskt kan ladda upp foton och videor med mixad verklighet från enhetens mapp Pictures > Camera Roll till motsvarande OneDrive för arbets- eller skolmappen. Den här funktionen åtgärdar ett funktionsgap i [OneDrive-appen](holographic-photos-and-videos.md#share-your-mixed-reality-photos-and-videos) på HoloLens 2, som endast stöder automatisk överföring av kamerarullen till en kunds personliga Microsoft-konto (och inte deras arbets- eller skolkonto).
...
- Upload pausas om enheten förfaller eller är avstängd. För att säkerställa att dina väntande uppladdningar slutförs använder du aktivt enheten tills Inställningar-sidan läser "OneDrive är uppdaterad" eller justerar **power &-viloläge.**
...
>[!NOTE]
>Idag har Microsoft HoloLens 2 stöd för månatliga underhållsuppdateringar (bugg- och säkerhetskorrigeringar) för följande versioner:
>
...
>
> Med introduktionen av Windows Holographic version 21H1 erbjuder vi månatliga underhållsuppdateringar **för Windows Holographic version 1903**. Detta gör att vi kan fokusera på nyare versioner och fortsätta att leverera värdefulla förbättringar.
...
Den nya Microsoft Edge [inför projektet Chromium öppen](https://blogs.windows.com/windowsexperience/2018/12/06/microsoft-edge-making-the-web-better-through-more-open-source-collaboration/) källkod för att skapa bättre kompatibilitet för kunder och mindre fragmentering av webben för webbutvecklare.
...
I den här versionen introducerar vi en ny version av Inställningar appen. Den nya Inställningar-appen innehåller nya funktioner och utökade inställningar för HoloLens 2 i följande områden: Ljud, & Strömsparläge, Network & Internet, appar, konton, Hjälpmedel med mera.
...
- System > Strömsparläge & strömsparläge: Välj när enheten ska förströms i strömsparläge efter en tids inaktivitet.
...
- Tidigare placerade Inställningar tas bort (se anmärkning ovan).
...
- På sidan Inställningar visas statussträngen som anger när färgprofilen senast ändrades tills du läser in sidan på Inställningar.
...
1. Öppna HoloLens Startskärmen med [startgesten eller](hololens2-basic-usage.md#start-gesture) säga "Gå till start".
...
I äldre versioner gäller att om en enhet har en kioskkonfiguration, vilket är en kombination av både global tilldelad åtkomst och AAD-gruppmedlem tilldelad åtkomst, om det inte gick att fastställa om AAD-gruppmedlemskap misslyckades, skulle användaren se["nothing shown in start"](hololens-kiosk.md#issue---no-apps-are-shown-in-start-menu-in-kiosk-mode)(inget visas på startmenyn).

Från och med Windows den här versionen kommer helskärmsupplevelsen att gå tillbaka till den globala helskärmskonfigurationen (om sådan finns) i händelse av fel i AAD-grupp helskärmsläge.
...
Vi utökar listan över tillgängliga URI:Inställningar som IT-administratörer kan hantera. Vissa av dessa URI:er är för nyligen tillgängliga områden i den nya Inställningar appen. Om du använder principen Inställningar/PageVisibilityList granskar du följande lista och justerar dina tillåtna eller blockerade sidor efter behov.
...
| Nätverksläge & internet > flygplansläge                   | `ms-settings:network-airplanemode`                 |
| Sekretessinställningar > allmänt                                    | `ms-settings:privacy-general`                      |
...
✔️ [**Förbättrad informationsdatorläge – fel vid leverans:**](#kiosk-mode-behavior-changes-for-handling-of-failures)

Om AAD-gruppmedlemskapet för den inloggade AAD-användaren inte har fastställts används den globala helskärmskonfigurationen för Start-menyn (om sådan finns), annars visas en tom startmeny. Även om den tomma startmenyn inte är en konfiguration som du kan ställa in direkt kan den här nya hanteringen vara något som informerar supportavdelningen om om du använder informationsdator, eftersom detta kan gälla för dina konfigurationer eller om du vill göra nya justeringar i dina tilldelade åtkomstkonfigurationer.
...
Om du för närvarande använder [Page Inställningar Visibility](settings-uri-list.md) kan du göra justeringar i dina befintliga URI:er som du antingen har tillåtit eller blockerat.
...
Du hittar exempelappar på de [universella Windows på GitHub](https://github.com/microsoft/Windows-universal-samples/tree/master/Samples) för att testa det här flödet.
...
| [Använda Autopilot med Wi-Fi anslutning](hololens-release-notes.md#using-autopilot-with-wi-fi-connection) | Använd Autopilot från enheten Wi-Fi utan behov av Ethernet-adapter. |
...
Med den här nya funktionen kan EN IT-administratör konfigurera en HoloLens 2-enhet för flera appar i helskärmsläge som gäller på systemnivå, som inte har någon tillhörighet till någon identitet i systemet och som gäller för alla som loggar in på enheten. Läs mer om den här nya funktionen i [HoloLens helskärmsläge](hololens-kiosk.md).
...
- Enhetshanteringsalternativ som är HoloLens för att hantera enheten.
...
I forskningsläge blir HoloLens 2 ett verktyg för forskning inom datorseende. Jämfört med tidigare utgåvor har Forskningsläge för HoloLens 2 följande fördelar:
...
- Ett problem har åtgärdats med start av Photos första startprogrammet efter uppdatering från 1903-versionen.
...
- Åtgärdat ett problem som gjorde HoloLens som ändrade pixelformatet till att återges svart i HoloLens Emulator.
