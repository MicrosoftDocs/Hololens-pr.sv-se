...
description: Lär dig hur du förbereder för registrering HoloLens 2 enheter i ett företagsanslutet nätverk med Dynamics 365-guider.
...
Dynamics 365-guider är ett molnbaserat program. Om nätverksadministratören har en lista över godkännanden kan han eller hon behöva lägga till IP-adresser och/eller slutpunkter som krävs för att ansluta till Dynamics 365-servrarna. [Läs mer om hur du avblockerar IP-adresser och URL:er.](/power-platform/admin/online-requirements#ip-addresses-and-urls)
...
Du måste också publicera din NDES-URL externt till företagsnätverket med [hjälp av Azure AD-programproxy eller Webbåtkomstproxy.](/azure/active-directory/manage-apps/application-proxy-add-on-premises-application) Du kan också använda en annan valfri omvänd proxy.

![SCEP-dataflöde.](./images/hololens2-scep-info-flow.png)

Om nätverket inte redan stöder SCEP, eller om du är osäker på om nätverket är korrekt konfigurerat för SCEP med Intune, kan du gå till Konfigurera infrastrukturen för att stödja SCEP med [Intune.](/mem/intune/protect/certificates-scep-configure)
...
## <a name="pkcs"></a>PKCS
Intune stöder också användning av PKCS-certifikat (privata och offentliga nyckelpar). Mer [information finns i Använda certifikat för privata och offentliga nycklar Microsoft Intune](/mem/intune/protect/certificates-pfx-configure) certifikat.
...
Flera appar kan installeras via Microsoft Store, men det är troligt att du har en egen anpassad app som du har skapat specifikt för att använda i mixad verklighet. Dessa anpassade appar som distribueras i hela organisationen för din verksamhet kallas verksamhetsapplikationer (LOB).
  
Det finns flera sätt att distribuera program till HoloLens 2 enheter. Appar kan distribueras direkt via MDM, Microsoft Store för företag(MSfB) eller separat inläst via ett etableringspaket. För den här guidens skull kommer vi att distribuera appar via MDM med hjälp av nödvändiga appinstallationer. Detta gör att dina LOB-appar kan laddas ned automatiskt till dina HoloLens enheter när de har avslutat registreringen.

För dig som inte har en egen LOB tillhandahåller vi en exempelapp för att testa det här distributionsflödet. Den här appen kommer att [vara appen MRTK Examples](https://aka.ms/HoloLensDocs-Sample-MRTK-Examples-App) och har redan förbyggts och paketerats för att testa konceptbevis.
